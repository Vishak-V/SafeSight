...
def get_analysis_from_api(base64_image):
    ...
    analysis = response_data['candidates'][0]['content']['parts'][0]
    
    # 'analysis' will be a dict if 'responseMimeType' works, 
    # or we might need to json.loads(analysis['text']) if it's a string
    if 'text' in analysis:
        # Model returned a JSON string
        return json.loads(analysis['text'])
    else:
        # Model returned a direct JSON object (Gemini 1.5+ capability)
        # This part is future-proofing, but response.json() often handles it.
        # For simplicity, we assume the API (with responseMimeType) gives us
        # the JSON object directly or as the 'text' string.
        # Let's refine based on the expected Gemini JSON output:
    ...
    try:
        while True:
            ...
            try:
                ...
                print("Model may have returned an invalid JSON. Skipping frame.")
            except Exception as e:
